...
export class AdministradorService {
  ...
  filtrar(parametros: any) {
    ...
    return this.http.get<any>(`${_SERVER}/api/admin/filtrar/0/5`, 
      { params: params }
    );
  }

  guardar(datos: TipoAdministrador, id?: number) {
    delete datos.id; 
    console.log(datos);
    if (id) {
  ...
  buscar(id: number) {
    return this.http.get<TipoAdministrador>(`${_SERVER}/api/admin/buscar/${id}`)
  }
