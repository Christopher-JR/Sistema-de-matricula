<?php
  ...
  class Usuario extends Autenticar {
    ...
    private function editarUsuario(string $idUsuario, int $rol = -1, string $passw = "") {
      $sql = $rol == -1
            ? "CALL passwUsuario(:idUsuario, :passw);"
            : "CALL rolUsuario(:idUsuario, :rol);";
    ...
    public function changePassw(Request $request, Response $response, $args) {
      ...
      if ($this->autenticar($args['idUsuario'], $body->passw, true)) {
        $passN = password_hash($body->passwN, PASSWORD_BCRYPT, ["cost" => 10]);
        $resp = $this->editarUsuario(idUsuario: $args['idUsuario'], passw: $passN);
        $status = 200;
      } else {
        $status = 401;
      }
    ...
    public function resetPassw(Request $request, Response $response, $args) {
      ...
      $status = $this->editarUsuario(idUsuario: $args['idUsuario'], passw: $passwN) == 0 ? 404 : 200;
      return $response->withStatus($status);
    }
    ...
    public function changeRol(Request $request, Response $response, $args) {
      ...
      $status = $this->editarUsuario(idUsuario: $args['idUsuario'], rol: $body->rol) == 0 ? 404 : 200;
      return $response->withStatus($status);
    }
